...
void MatrixChainMultiplication(int p[], int n) {
    ...
    // Initialize the matrices m and s
    for (int i = 1; i < n; i++) {
        for (int j = 1; j < n; j++) {
            m[i][j] = 0;
            s[i][j] = 0;
    ...
    for (int d = 1; d < n - 1; d++) {  // The number of matrices to be multiplied
        for (int i = 1; i <= n - d - 1; i++) { // Starting matrix
            int j = i + d; // Ending matrix
            int min = INT_MAX; // Setting minimum as a random large value to compare
            for (int k = i; k < j; k++) {
            ...
            m[i][j] = min; // Assigning the m matrix the minimum number of multiplications possible value
        }
    }
